...
func main() {
	...
	if err != nil || atmpt <= 0 {
		http.Error(w, "Attempts must be a positive number.", http.StatusBadRequest)
		return
	}

	depthInt, err := strconv.Atoi(depth)
	if err != nil || depthInt < 0 {
		http.Error(w, "Depth must be a non-negative number.", http.StatusBadRequest)
		return
	}
...
func monitor_results(topic string) {
	for {
		...
		if sr.Success {
			...
			defer f.Close()
			//if _, err := f.WriteString(fmt.Sprintf("<p>%s: %s %s <a href=\"https://gateway.ipfs.io/ipfs/%s\">%s</a></p>\n", sr.Host, sr.CrawlVersion, sr.Seed, sr.IPFSHash, sr.IPFSHash)); err != nil {
			...
			/*err = ioutil.WriteFile(fmt.Sprintf("results/%s.txt",sr.Uuid), sr.Output, 0644)
			if err != nil {
				fmt.Println("Failed to create file for",sr.Uuid)
				continue
			}
			*/
		} else {
			...
			defer f.Close()
			if _, err := f.WriteString(fmt.Sprintf("<p>%s: %s %s gave up</a></p>\n", sr.Host, sr.CrawlVersion, sr.Seed)); err != nil {
